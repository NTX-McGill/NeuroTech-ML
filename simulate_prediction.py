...
where the data came from, but we're not doing that currently because this 
portion needs to be updated to be consistent with the new way select_files() works
******
""" 
# take all ok and good data files with modes 3 and 4
selected_files = select_files(path_data='data', path_trials_json='siggy', trial_groups=['ok', 'good'])

#selected_files = select_files(path_data='data', path_trials_json='siggy', modes=[3,4], trial_groups=['ok', 'good'])
